...
from whisperx_batch.batch import transcribe_task

__all__ = ["transcribe_task"]


def main():
    parser = argparse.ArgumentParser(
        description="WhisperX batch processing with Ray")

    # new params for ray
    parser.add_argument("--batch_file", type=str, default=None,
                        help="path to a file containing a list of audio files to transcribe", required=True)
    parser.add_argument("--load_audio_num_tasks", type=float, default=2,
                        help="number of tasks to use for loading audio into memory")
    parser.add_argument("--load_audio_num_cpus", type=float, default=0.5,
                        help="number of CPUs to use for loading audio into memory")
    parser.add_argument("--transcribe_actor_size", type=int,
                        default=1, help="number of alignment actors to use")
    parser.add_argument("--transcribe_num_gpus", type=float, default=0.4,
                        help="number of GPUs to use for transcription actors")
    parser.add_argument("--transcribe_num_cpus", type=float, default=1,
                        help="number of CPUs to use for transcription actors")
    parser.add_argument("--align_actor_size", type=int,
                        default=2, help="number of alignment actors to use")
    parser.add_argument("--align_num_gpus", type=float, default=0.3,
                        help="number of GPUs to use for alignment actors")
    parser.add_argument("--align_num_cpus", type=float, default=1,
                        help="number of CPUs to use for alignment actors")
    parser.add_argument("--diarize_actor_size", type=int,
                        default=1, help="number of alignment actors to use")
    parser.add_argument("--diarize_num_gpus", type=float, default=0.1,
                        help="number of GPUs to use for diarization actors")
    parser.add_argument("--diarize_num_cpus", type=float, default=4,
                        help="number of CPUs to use for diarization actors")
    ...
    parser.add_argument("--compute_type", default="float16", type=str, choices=[
                        "float16", "float32", "int8"], help="compute type for computation")

    parser.add_argument("--output_dir", "-o", type=str,
                        default=".", help="directory to save the outputs")
    parser.add_argument("--output_format", "-f", type=str, default="all", choices=[
                        "all", "srt", "vtt", "txt", "tsv", "json", "aud"], help="format of the output file; if not specified, all available formats will be produced")
    parser.add_argument("--verbose", type=str2bool, default=False,
                        help="whether to print out the progress and debug messages")
    parser.add_argument("--log-level", type=str, default=None, choices=[
                        "debug", "info", "warning", "error", "critical"], help="logging level (overrides --verbose if set)")
    ...
    parser.add_argument("--language", type=str, default=None, choices=sorted(LANGUAGES.keys()) + sorted(
        [k.title() for k in TO_LANGUAGE_CODE.keys()]), help="language spoken in the audio, specify None to perform language detection", required=True)

    # alignment params
    parser.add_argument("--align_model", default=None,
                        help="Name of phoneme-level ASR model to do alignment")
    parser.add_argument("--interpolate_method", default="nearest", choices=[
                        "nearest", "linear", "ignore"], help="For word .srt, method to assign timestamps to non-aligned words, or merge them into neighbouring.")
    ...
    parser.add_argument("--diarize_model", default="pyannote/speaker-diarization-3.1",
                        type=str, help="Name of the speaker diarization model to use")
    parser.add_argument("--speaker_embeddings", action="store_true",
                        help="Include speaker embeddings in JSON output (only works with --diarize)")
    ...
    parser.add_argument("--hotwords", type=str, default=None,
                        help="hotwords/hint phrases to the model (e.g. \"WhisperX, PyAnnote, GPU\"); improves recognition of rare/technical terms")
    ...
    parser.add_argument("--segment_resolution", type=str, default="sentence", choices=[
                        "sentence", "chunk"], help="(not possible with --no_align) the maximum number of characters in a line before breaking the line")
    ...
    parser.add_argument("--version", "-V", action="version", version=f"%(prog)s {
                        importlib.metadata.version('whisperx')}", help="Show whisperx version information and exit")
    parser.add_argument("--python-version", "-P", action="version", version=f"Python {platform.python_version()} ({
                        platform.python_implementation()})", help="Show python version information and exit")
